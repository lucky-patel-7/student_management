...
class Teachers {
  ...
  static getStudentMonthlyScore = asyncWrapper(async (req, res) => {
   ...
   console.log("type", typeof (month))
    ...
    if (!monthlyScore || monthlyScore.length === 0) {
      let data = Response(Constants.RESULT_CODE.ERROR, Constants.RESULT_FLAG.FAIL, "No monthly score found for this student", "");
      return res.send(data);
    }

    let data = Response(Constants.RESULT_CODE.OK, Constants.RESULT_FLAG.SUCCESS, "Monthly score of  student", monthlyScore);
    return res.send(data);
  });
  ...
  static totalTimeSpent = asyncWrapper(async (req, res) => {
        ...
        $project: {
          timeDifference: {
            $divide: [
              ...
              1000 * 60 * 60, // Convert milliseconds to minutes
            ],
          },
  ...
  static isLateCount = asyncWrapper(async (req, res) => {
    ...
    const promises = students.map(async (student) => {
      const count = await Attendance.countDocuments({ studentId: student._id, isLate: true });
      student.count = count;
      return student;
    });
